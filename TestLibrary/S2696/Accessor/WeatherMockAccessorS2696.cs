...
public class WeatherMockAccessor: WeatherAccessorBase
{
  ...
  {
    int callCount = CalculateCallCount(argument);
  ...
  private static int CalculateCallCount(string? argument)
  {
    return string.IsNullOrWhiteSpace(argument) ? 0 : argument.GetHashCode();
  }
}
