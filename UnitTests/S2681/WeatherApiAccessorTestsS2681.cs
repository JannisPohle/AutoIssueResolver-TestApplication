...
using TestLibrary.S2681.Models.External;
...
namespace UnitTests.S2681;

[Trait("ShouldFail", "true")] // Expect these tests to fail currently, as the orchestrator will always return with a failure due to the sonarqube issue S2681
public partial class WeatherOrchestratorTests
{
  ...
  private void SetupWireMockServer()
  {
    ...
    _wireMockServer?.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.RejectOnMatch).UsingGet())
                   .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S2681.Models.External.WeatherApiModel>
                   {
