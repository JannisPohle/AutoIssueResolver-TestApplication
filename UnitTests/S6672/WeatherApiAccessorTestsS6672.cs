...
using TestLibrary.S6672.Models.External;
...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_GetDefaultValues_ShouldLog()
  {
    // Arrange
    SetupWireMockServer();
    ...
    _apiAccessorLogger.Received(1).Log(LogLevel.Information, Arg.Any<EventId>(),
                                       Arg.Is<object>(v => v.ToString()!.Contains("Found 3 weather data for location")),
                                       Arg.Any<Exception>(),
  ...
  private void SetupWireMockServer()
  {
    ...
    _wireMockServer?.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.RejectOnMatch).UsingGet())
                   .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S6672.Models.External.WeatherApiModel>
                   {
