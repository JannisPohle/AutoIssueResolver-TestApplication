...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_Mock_WithArgument_NoDataFound_ShouldThrowException_AndLogWarning()
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Mock, "0");
    ...
    _mockAccessorLogger.Received(1).Log(LogLevel.Warning, Arg.Any<EventId>(),
                                        Arg.Is<object>(v => v.ToString()!.Contains("No weather data found for")),
                                        Arg.Any<Exception>(),
