...
public sealed record WeatherApiQuery(string? Location, string? StartTime, string? EndTime, string? Longitude, string? Latitude, string? Unit);
...
public sealed class WeatherApiAccessor(ILogger<WeatherApiAccessor> logger): WeatherAccessorBase(logger), IDisposable
{
  ...
  public async Task<List<WeatherModelCelsius>> GetWeather(WeatherApiQuery query)
  {
    ...
    {
      Logger.LogWarning(e, "Failed to get weather data with arguments:  {Query}", query);

      throw new ConnectionFailedException($"Failed to connect to the weather API with arguments: {query}.", e);
    }
  }

  public override async Task<List<WeatherModelCelsius>> GetWeather(string? argument)
  {
    return await GetWeather(new WeatherApiQuery(argument, null, null, null, null, null));
  }
