...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_File_WithArgument_FileNotFound_ShouldFail_AndLogWarning()
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.File, $"TestFiles/{Guid.NewGuid()}.json");
    ...
    _fileAccessorLogger.Received(1).Log(LogLevel.Warning, Arg.Any<EventId>(),
                                        Arg.Is<object>(v => v.ToString()!.Contains("Weather data file not found:")),
                                        Arg.Any<Exception>(),
