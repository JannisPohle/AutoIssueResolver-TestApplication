...
using TestLibrary.S2198.Models.External;
...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_ValueOutOfRange()
  {
    // Arrange
    SetupWireMockServer();
    ...
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeFalse();
    result.Exception.Should().BeAssignableTo<ValidationException>();

    _wireMockServer!.Should().HaveReceivedACall().AtUrl("http://localhost:31246/v1/api/weather?location=Bergen").And.UsingGet();
  }

  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_LargeValue()
  {
    // Arrange
    SetupWireMockServer();
    ...
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Web, "Madrid");

    // Assert
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeTrue();
    result.Payload.Should().NotBeNullOrEmpty().And.HaveCount(1);
    result.Payload.Should().Satisfy(w => w.Temperature == int.MaxValue);

    _wireMockServer!.Should().HaveReceivedACall().AtUrl("http://localhost:31246/v1/api/weather?location=Madrid").And.UsingGet();
  }
