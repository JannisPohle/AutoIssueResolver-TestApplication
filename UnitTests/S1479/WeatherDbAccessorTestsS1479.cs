...
public partial class WeatherOrchestratorTests
{
  ...
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Database.ToString());
  ...
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Database.ToString(), "Database/InvalidDBFile.db");
