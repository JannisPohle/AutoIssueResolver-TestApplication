...
using TestLibrary.S4019.Models.External;
...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_SingleValue_OutOfRange()
  {
    // Arrange
    SetupWireMockServer();
    _wireMockServer!.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.AcceptOnMatch, "SaoPaulo").UsingGet())
                    .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S4019.Models.External.WeatherApiModel>
                    {
    ...
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeFalse();
    result.Exception.Should().NotBeNull().And.BeAssignableTo<ArgumentException>().Which.Message.Should().Contain("Invalid weather data encountered");
