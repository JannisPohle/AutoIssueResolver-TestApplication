...
public class WeatherOrchestrator: IWeatherOrchestrator
{
  ...
  public async Task<Result<List<WeatherModelCelsius>>> GetWeather(AccessMode mode, string? argument = null)
  {
    try
    {
      ...
      List<WeatherModelCelsius> result = await FetchWeatherData(mode, argument);
  ...
  private async Task<List<WeatherModelCelsius>> FetchWeatherData(AccessMode mode, string? argument)
  {
    return mode switch
    {
