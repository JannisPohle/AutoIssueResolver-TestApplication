...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_DbAccessor_WithArgument_InvalidConnectionString_ShouldFailAndLogError()
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Database, "Database/InvalidDBFile.db");
    ...
    _dbAccessorLogger.Received(1).Log(LogLevel.Error, Arg.Any<EventId>(),
                                      Arg.Is<object>(v => v.ToString()!.Contains("Failed to open database connection with argument:")),
                                      Arg.Any<Exception>(),
