...
using TestLibrary.S3776.Models.External;
...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_SingleValue_OutOfRange_TooHigh()
  {
    // Arrange
    SetupWireMockServer();
    _wireMockServer!.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.AcceptOnMatch, "SaoPaulo").UsingGet())
                    .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S3776.Models.External.WeatherApiModel>
                    {
                      new() { Location = "SaoPaulo", Temperature = 125, Condition = "Sunny" },
                    }));

    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Web, "SaoPaulo");

    // Assert
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeFalse();
    result.Exception.Should().NotBeNull().And.BeAssignableTo<ValidationException>().Which.Message.Should().Contain("Temperature cannot exceed");

    _wireMockServer!.Should().HaveReceivedACall().AtUrl("http://localhost:31246/v1/api/weather?location=SaoPaulo").And.UsingGet();
  }

  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_SingleValue_OutOfRange_TooLow()
  {
    // Arrange
    SetupWireMockServer();
    _wireMockServer!.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.AcceptOnMatch, "Bergen").UsingGet())
                    .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S3776.Models.External.WeatherApiModel>
                    {
                      new() { Location = "Bergen", Temperature = -105, Condition = "Sunny" },
                    }));

    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.Web, "Bergen");

    // Assert
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeFalse();
    result.Exception.Should().NotBeNull().And.BeAssignableTo<ValidationException>().Which.Message.Should().Contain("Temperature cannot be below");

    _wireMockServer!.Should().HaveReceivedACall().AtUrl("http://localhost:31246/v1/api/weather?location=Bergen").And.UsingGet();
  }
