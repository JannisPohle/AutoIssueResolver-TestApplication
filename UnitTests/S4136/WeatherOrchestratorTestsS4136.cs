...
using TestLibrary.S4136;
using TestLibrary.S4136.Accessor;
using TestLibrary.S4136.Models;
using WireMock.Server;
using Xunit;

namespace UnitTests.S4136;
...
public sealed partial class WeatherOrchestratorTests: IDisposable
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_AccessModeNone_ShouldFail_And_ThrowException()
  {
    // Act
    var action = async () => await _weatherOrchestrator.GetWeather(AccessMode.None, null, true);
  ...
  [Fact]
  public async Task WeatherOrchestrator_AccessModeNone_ShouldFail_And_NotThrowException()
  {
    // Act
    var action = async () => await _weatherOrchestrator.GetWeather(AccessMode.None, null, false);
