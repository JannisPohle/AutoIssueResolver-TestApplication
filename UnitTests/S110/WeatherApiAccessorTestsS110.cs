...
using TestLibrary.S110.Models.External;
...
public partial class WeatherOrchestratorTests
{
  ...
  [Fact]
  public async Task WeatherOrchestrator_ApiAccessor_WithParam_InvalidValue_ShouldLogWarning()
  {
    // Arrange
    SetupWireMockServer();
    ...
    _apiAccessorLogger.Received(1).Log(LogLevel.Warning, Arg.Any<EventId>(),
                                       Arg.Is<object>(v => v.ToString()!.Contains("Failed to get weather data with argument:")),
                                       Arg.Any<Exception>(),
  ...
  private void SetupWireMockServer()
  {
    ...
    _wireMockServer?.Given(Request.Create().WithPath("/v1/api/weather").WithParam("location", MatchBehaviour.RejectOnMatch).UsingGet())
                   .RespondWith(Response.Create().WithStatusCode(200).WithBodyAsJson(new List<TestLibrary.S110.Models.External.WeatherApiModel>
                   {
