...
public sealed class WeatherApiAccessor(ILogger<WeatherApiAccessor> logger): WeatherAccessorBase(logger), IDisposable
{
  ...
  public async Task<List<WeatherModelCelsius>> GetWeather(WeatherRequestParameters parameters)
  {
    try
    {
    ...
    {
      Logger.LogWarning(e, "Failed to get weather data with parameters:  {Parameters}", parameters);

      throw new ConnectionFailedException($"Failed to connect to the weather API with parameters: {parameters}.", e);
    }
  }

  public override async Task<List<WeatherModelCelsius>> GetWeather(string? argument)
  {
    return await GetWeather(new WeatherRequestParameters { Location = argument });
  }
