...
namespace TestLibrary.S2365.Accessor;
...
public sealed class WeatherApiAccessor(ILogger<WeatherApiAccessor> logger): WeatherAccessorBase(logger), IDisposable
{
  ...
  public override async Task<List<WeatherModelCelsius>> GetWeather(string? argument)
  {
    try
    {
      ...
      var response = await _httpClient.GetFromJsonAsync<WeatherApiModelResult>(url);
      ...
      var weatherData = response.GetItems().Select(weatherModel => new WeatherModelCelsius((int) weatherModel.Temperature)).ToList();
