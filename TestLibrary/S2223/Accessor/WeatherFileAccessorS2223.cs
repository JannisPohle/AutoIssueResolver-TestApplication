...
public class WeatherFileAccessor: WeatherAccessorBase
{
  ...
  /// <inheritdoc />
  public override async Task<List<WeatherModel>> GetWeather(string? argument)
  {
    var stringContent = await ReadFromFile(argument ?? "TestFiles/WeatherForecast.json");
    var weather = JsonSerializer.Deserialize<IEnumerable<WeatherModel>>(stringContent, JsonSerializerOptions.Web)?.ToList();
