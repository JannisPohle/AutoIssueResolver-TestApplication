...
public partial class WeatherOrchestratorTests
{
  ...
  {
    // Act
    var result = await _weatherOrchestrator.GetWeather(AccessMode.File, "TestFiles/S3776/OutOfRangeWeatherForecast.json");

    // Assert
    result.Should().NotBeNull();
    result.IsSuccess.Should().BeFalse();
    result.Exception.Should().NotBeNull().And.BeAssignableTo<ValidationException>().Which.Message.Should().Contain("Temperature cannot");
  }
}
